...
def run_fkpp_for_participant(clinical_group, 
                        participant, 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--constrain_seeds", type=bool, required=False, default=True, help="End participant index (exclusive)")
    parser.add_argument("--n_calls", type=int, required=False, default=128, help="End participant index (exclusive)")
    ...
    if args.constrain_seeds:
        seed_list = constants.fkpp_seed_list
    else:
        seed_list = None
    ...
    tasks = [
        delayed(run_fkpp_for_participant)(group, participant, seed_list=seed_list, n_calls=n_calls)
    ...
    results = Parallel(n_jobs=-1)(
        tqdm(tasks, total=total_tasks, desc="Running FKPP")
    )
